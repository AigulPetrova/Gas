...
%%
classes = ["0" "1"]; %0 - нет отклонения, 1 - есть отклоенение
%%
YY = D{:,end};
%%
...
% figure
% plot(y_pred');
%% предсказание по нарастанию вероятности  
temp = false;
pr = 0.5;
dur = 0;
n=1;
net = resetState(net);% Сброс состояния сети предотвращает влияние предыдущих прогнозов на прогнозы новых данных. 
% y_pred = zeros(2, numel(y_test));
for i = 1:numel(y_test)
    [net,y_pred(:,i)] = predictAndUpdateState(net,x_train(:,i)); % затем инициализируйте состояние сети, предсказав обучающие данные.
% сюда трешхолды по вероятности 
    if y_pred(i) > pr
        temp = true;
        if dur == 0
            n=n+1;
        ...
        if temp 
        dur = dur +1;
        disp("Отклонение,"  + " " + "индекс" + " " + string(i) + " " + "номер" + " " + string(n) + " " + "значение" + " " + y_pred(i))
...
figure
yyaxis left
plot(D{N_train+1:end,4},'b', LineWidth=2);
yyaxis right
plot(y_pred(2,:),'g', LineWidth=3);
hold on
plot(double(y_test)-1,'k', LineWidth=1);
title("Сравнение предсказаний, разметки и реальных данных");
legend ("Реальные данные", "Предсказания", "Разметка");
%%
% как предсказывать заранее? +
% confusionmat(y_test,y_pred_test); +
% regression - как соединить с предыдущей lstm? +
% как автоматически размечать уровни, программно менять уровни? + 
...
% можно ли сделать нейросеть для всего баланса или она  для этого не нужна?
