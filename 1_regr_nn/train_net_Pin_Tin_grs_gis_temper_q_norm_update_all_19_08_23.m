...
%     %было x{i} = [x{i}(:,2:end)];
% end
%% считаем матожидание и ско
for i=1:numel(x)% число ячеек - грс в массиве х
    for j = 2:5  % число нормируемых входных  переменных в каждой грс в массиве х
    m(j) = mean(x{i, 1}(j,1:end));
    s(j) = std(x{i, 1}(j,1:end));
    end
end
%% нормируем x
for i=1:numel(x)% число ячеек - грс в массиве х
    for j = 2:5 % число нормируемых строк-входных  переменных в каждой грс в массиве х
    x{i, 1}(j,1:end) = (x{i, 1}(j,1:end) - m(j))./s(j);
    end
end
%% проверка нормирования x, m должно получиться около нуля, s1 около 1
for i=1:numel(x)%
    for  j = 2:5  % число строк-входных  переменных в каждой грс в массиве х
    m1(j) = mean(x{i, 1}(j,1:end));
    s1(j) =std (x{i, 1}(j,1:end));
    end
end
%% формируем строку ответов, это расход
for i = 1:numel(x)
...
%%
x_train{17} = x{17}(:,1:8000);% обучающая выборка для входного параметра, часть из 4 ГРС
y_train{17} = y{17}(1:8000); % обучающая выборка для выходного параметра, часть из 4 ГРС
%%
figure
plot(x_train{1}');
...
ops = trainingOptions("adam","Plots","training-progress",'MaxEpochs',100, "ValidationData", {x_test, y_test}, ...
    "InitialLearnRate", 0.01, "LearnRateSchedule", "piecewise", "LearnRateDropPeriod", 40, "LearnRateDropFactor", 0.3,'MiniBatchSize', 1);
%%
