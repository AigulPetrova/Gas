...
%%
% idx = [idx_1,idx_2,idx_3,idx_4,idx_3_1,idx_3_2,idx_3_4,idx_3_6,idx_5_1,idx_5_2,idx_8_1,idx_8_2];
% for i = 5:16
%     x{i} = [t1(1:idx(i-4),2)';x{i}];
%     x{i} = [t1(1:idx(i-4),1)';x{i}];
% end
%%    
%%
...
%% Получаем параметры нормировки для x
for i=1:numel(x_train)% число ячеек - грс в массиве х
    ...
    m(j) = mean(x_train{i, 1}(j,1:end));
    s(j) = std(x_train{i, 1}(j,1:end));
    end
end
%% Параметры для y
...
%%
ops = trainingOptions("adam","Plots","training-progress",'MaxEpochs',1000, "ValidationData", {x_val, y_val}, ...
